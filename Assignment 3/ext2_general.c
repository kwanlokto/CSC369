...
void init_datastructures() {
	...
	printf("blocksize %lu disk %lu\n", block_size, EXT2_BLOCK_SIZE);
	descriptor = (struct ext2_group_desc *)(block + block_size);

	//block number where the inode table starts
	inode_table_block_no = descriptor -> bg_inode_table;
	//Reference from the table 3-1 in nongnu.org
	inode_table = (struct ext2_inode *)(block + (inode_table_block_no - 1) * block_size);

	unsigned int n =descriptor->bg_inode_bitmap;
	while (n) {
    if (n & 1)
        printf("1");
    else
...
printf("\n");
	//printf("inode bitmap: %d", binary_conversion(descriptor->bg_inode_bitmap));
}
...
unsigned int check_directory(char * name, unsigned int block_no, unsigned int (*fun_ptr)(unsigned int, char *)){
	...
	//struct ext2_inode * inode = (struct ext2_inode *)(block + (block_no - 1) * block_size);
	...
	while (curr_block_no != 0) {
		...
		if ((return_val = (*fun_ptr)(curr_block_no, name)) > 0) {
			return return_val;
		}
...
unsigned int print_file(unsigned int block_no, char * name) {
	struct ext2_dir_entry_2 * curr_block = (struct ext2_dir_entry_2 *)(block + (block_no - 1) * block_size);
	printf("%s\n", curr_block->name);
	return 0;
}
...
unsigned int check_entry(unsigned int block_no, char * name){
	...
	struct ext2_dir_entry_2 * i_entry = (struct ext2_dir_entry_2 *)(block + (block_no - 1) * block_size);
...
unsigned int find_singly_indirect(int block_no, int i){
	if (block_no != 0) {
		unsigned int * singly_indirect = (unsigned int *)block + (block_no - 1) * block_size;
...
unsigned int find_doubly_indirect(int block_no, int i, int j) {
	if (block_no != 0) {
		unsigned int * doubly_indirect = (unsigned int *)block + (block_no - 1) * block_size;
...
unsigned int find_triply_indirect(int block_no, int i, int j, int k) {
	if (block_no != 0) {
		unsigned int * triply_indirect = (unsigned int *)block + (block_no - 1) * block_size;
