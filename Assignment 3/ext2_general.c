	...
	if (block_no != 0) {
		...
		if (checking_free) {
			...
			if (new_rec_len < EXT2_BLOCK_SIZE) {
				...
				//i_entry = (struct ext2_dir_entry_2)((char*))i_entry + current_size;
...
int add_entry_in_directory(struct ext2_inode * dir_inode, unsigned int new_inode, char* entry_name, int entry_type)
{
