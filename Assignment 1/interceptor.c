...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	int check = check_pid_monitored(reg.ax, current->pid);
	spin_lock(&pidlist_lock);
	if ((check == 1 && monitored == 1) || (check == 0 && monitored == 2)) {
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	if (cmd != REQUEST_SYSCALL_INTERCEPT && cmd != REQUEST_SYSCALL_RELEASE && cmd !=
		REQUEST_START_MONITORING && cmd != REQUEST_STOP_MONITORING) {
		return -EINVAL;
	}
	...
	*/
	if (syscall < 0 || syscall > NR_syscalls ||syscall == MY_CUSTOM_SYSCALL) {
		return -EINVAL;
	}	else if (pid != 0 && (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING)) {
  ...
  *        system call has not been intercepted yet.
	*/
	if (table[syscall].intercepted == 0 && (cmd == REQUEST_SYSCALL_RELEASE || cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING)) {
		return -EINVAL;
	} else if (check_pid_monitored(syscall, pid) == 0 && cmd == REQUEST_STOP_MONITORING && table[syscall].monitored == 1) { //whitelist
		return -EINVAL;
	} else if (check_pid_monitored(syscall, pid) == 1 && cmd == REQUEST_STOP_MONITORING && table[syscall].monitored == 2) { //blacklist
		return -EINVAL;
	}

	/* Handling each individual command
	 * Error checking for -ENOMEM and -EBUSY is handled here
	 */
	if (cmd == REQUEST_SYSCALL_INTERCEPT) {
		//Check if it is already intercepted
		...
		//Modify the system call table so at index syscall, it will contain a pointer to
		...
		spin_lock(&calltable_lock);
		set_addr_rw((unsigned long) sys_call_table);
		sys_call_table[syscall] = table[syscall].f; //Restore the original system call
		set_addr_ro((unsigned long) sys_call_table);
		spin_unlock(&calltable_lock);

	} else if (cmd == REQUEST_START_MONITORING){

		//Checks if we want to operate on all pids
		if (pid == 0) {
			spin_lock(&pidlist_lock);
			int monitored = table[syscall].monitored;

			//Checks if all pids are monitored
			if (table[syscall].listcount == 0 && monitored == 2) {
				spin_unlock(&pidlist_lock);
				return -EBUSY;
			}
			//Changes from whitelist to blacklist
			destroy_list(syscall);
			table[syscall].monitored = 2; //Need to do it after destroy list
			spin_unlock(&pidlist_lock);
		} else {
			...
			int monitored = table[syscall].monitored;
			int check = check_pid_monitored(syscall, pid);
			if ((check == 1 && monitored == 1) || (check == 0 && monitored == 2)) {
				spin_unlock(&pidlist_lock);
				return -EBUSY;
			}

			//Checks if this system call is currently not monitored
			if (table[syscall].monitored == 0) {
				table[syscall].monitored = 1;
			}
			//Case where monitored = 1 -- Whitelist
			if (table[syscall].monitored == 1) {
				if (add_pid_sysc(pid,syscall) != 0) { //Checks if insuccesful in adding pid
					spin_unlock(&pidlist_lock);
					return -ENOMEM;
				}
			}
			//Case where monitored = 2 -- Blacklist
			else {
		...
		//Checks if the current system call is monitored
		spin_lock(&pidlist_lock);
		if (table[syscall].monitored == 0) {
			spin_unlock(&pidlist_lock);
			return -EINVAL;
...
static int init_function(void) {
	...
	// don't start at 0 because table[0] represents this syscall we implementing
	int i = 1;
	spin_lock(&pidlist_lock);
...
{
	//Reset all the values to the original

	int i = 1;
	spin_lock(&calltable_lock);
	set_addr_rw((unsigned long)sys_call_table);
	while (i < NR_syscalls) {
		destroy_list(i); //want to free all allocated memory
		sys_call_table[i] = table[i].f;  //reset the table
		i++;
	}
